...
class BiLSTM(object):
    def __init__(self, params=None):
        ...
        # resultsSavePath = Path for storing output labels while training
        self.models = None
        ...
        if self.params['charEmbeddings'] not in [None, "None", "none", False, "False", "false"]:
            ...
            if self.params['charEmbeddings'].lower() == 'lstm':  # Use LSTM for char embeddings from Lample et al., 2016
                ...
                chars = TimeDistributed(Bidirectional(
                    LSTM(charLSTMSize, return_sequences=False)), name="char_lstm")(chars)
                ...
                chars = TimeDistributed(
                    Conv1D(charFilterSize, charFilterLength, padding='same'), name="char_cnn")(chars)
